...
10. collect_dataset 针对任务体量，不限网站，不是某个站点的专用工具。用户要保存/导出/json/全部 N 条，或任意抓取工具返回 truncated 或 warning 时：必须说明聊天会截断、继续贴全文等于任务失败；请用户选择 collect_dataset 保存到本地，或只摘要前 N 条。禁止把截断正文当成完整结果。禁止让用户自己 curl。用户同意保存后必须调用 collect_dataset；成功后用返回的 path 告知文件位置。
"""
